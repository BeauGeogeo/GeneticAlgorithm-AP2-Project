...
class HauntedField(object):
    ...
    def __init__(self, height, width):
        """
        haunted field is a 2D array of height x width char,
        initially all cells are EMPTY
        cells coordinates range from (1,1) to (height,width)
    ...
    def restore_field(self):
        """
        reset self's field to last 'backup field' (created using self.restore_field()), it is None if no backup had been
        made
        """
