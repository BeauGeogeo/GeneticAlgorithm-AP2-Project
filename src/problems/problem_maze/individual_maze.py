...
class IndividualMaze(IndividualInterface):
    MAX_OR_MIN = True

    def __init__(self, genome_length):
        self.__genome_length = genome_length
        super(IndividualMaze, self).__init__()
    ...
    def init_value(self):
        ...
        :rtype: NoneType
        """
        return "".join([random.choice(Maze.DIRECTIONS) for i in range(self.__genome_length)])
    ...
    def gene_mutation(gene):
        ...
        :rtype: str
        """
        return random.choice([genes for genes in Maze.DIRECTIONS if genes != gene])
